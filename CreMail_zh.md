...
- `log_file` (str, 可选): 日志文件路径，用于将日志写入文件。可以是绝对路径或相对路径（相对于当前工作目录）。默认: None，仅输出到控制台
...
该模块提供灵活的日志记录选项，具有用户友好的错误消息，包含故障排除建议。主要日志事件包括：
...
1. 使用CreMail初始化时的`log_file`参数将日志写入文件：
```python
from cremail import CreMail
...
改进的日志系统现在提供清晰、可操作的错误消息和故障排除提示，帮助用户快速识别和解决常见问题，例如：
- SMTP连接错误（检查服务器地址和端口）
- 认证错误（验证邮箱和密码/授权码）
- 收件人错误（验证邮箱地址）
- 附件错误（检查文件路径和大小）
